    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            ...
            if (i<sectionNum && i != 0) {
                ...
                lineView.backgroundColor = [UIColor redColor];//中间的竖线的颜色,分割线
...
{
    if (!self.mTableView) {
        //阴影部分的位置及颜色    the position and colour of the shadow part
        ...
        self.mTableBaseView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height , self.frame.size.width, 0)];
    ...
    CGRect rect = self.mTableView.frame;
    rect.origin.x = sectionWidth *section+[UIScreen mainScreen].bounds.size.width/3.5;//修改下来的tableView的位置
    rect.size.width = sectionWidth+[UIScreen mainScreen].bounds.size.width/23;;
    rect.size.height = 0;
    self.mTableView.frame = rect;
